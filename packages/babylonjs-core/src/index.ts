...
interface TileDBCloudPointOptions extends TileDBVisualizationBaseOptions {
...
export const renderCanvas = (options: {
  width: string;
  height: string;
  inspector?: boolean;
  rootElement: HTMLElement;
}) => {
  console.log('Rendering from core');
  const { width, height, inspector, rootElement } = options;
...
const resizeCanvas = (options: {
  canvas: HTMLCanvasElement;
  width: string;
  height: string;
  engine: Engine;
}) => {
  const { canvas, width, height, engine } = options;
...
// export const renderPointCloudVis = (rootElement: HTMLElement, options: PointCloudOptions) => {
//   renderCanvas
//   const scene = createScene({rootElement});
// };
